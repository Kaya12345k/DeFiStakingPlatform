...
func main() {
    ...
    http.HandleFunc("/getTotalStaked", getTotalStakedHandler) 
...
func getTotalStakedHandler(w http.ResponseWriter, r *http.Request) {
    if r.Method != "GET" {
        http.Error(w, "Only GET method is allowed", http.StatusMethodNotAllowed)
        return
    }
    ...
    response := map[string]float64{"totalStaked": totalStaked}
    w.Header().Set("Content-Type", "application/json")
